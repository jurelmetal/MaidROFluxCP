...
<?php if (Flux::config('TermsOfServiceExternalUrl')): ?>
<p><?php printf(htmlspecialchars(Flux::message('AccountCreateInfo')), '<a target="_blank" href="'.Flux::config('TermsOfServiceExternalUrl').'">'.Flux::message('AccountCreateTerms').'</a>') ?></p>
<?php else: ?>
<p><?php printf(htmlspecialchars(Flux::message('AccountCreateInfo')), '<a target="_blank" href="'.$this->url('service', 'tos').'">'.Flux::message('AccountCreateTerms').'</a>') ?></p>
<?php endif ?>
<?php if (Flux::config('RequireEmailConfirm')): ?>
<p><strong>Note:</strong> Tendrás que proveer una dirección de correo electrónico correcta para poder activar tu cuenta.</p>
<?php endif ?>
<p><strong>Note:</strong> <?php echo sprintf(Flux::message('PasswordTooShort'), Flux::config('MinPasswordLength'), Flux::config('MaxPasswordLength')) ?></p>
...
<?php endif ?>
<p><strong>Las direcciones de correo electrónico de Microsoft (hotmail.com, outlook.com, live.com) no están soportadas ya que presentan problemas al recibir correos de nuestro proveedor. Disculpamos las molestias.</strong></p>
	...
	<table class="generic-form-table">
		...
		<tr>
			...
			<td>
				...
				<?php if (Flux::config('TermsOfServiceExternalUrl')): ?>
					<?php printf(htmlspecialchars(Flux::message('AccountCreateInfo2')), '<a target="_blank" href="'.Flux::config('TermsOfServiceExternalUrl').'">'.Flux::message('AccountCreateTerms').'</a>') ?>
				<?php else: ?>
